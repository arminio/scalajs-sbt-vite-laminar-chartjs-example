...
trait LaminarSpec
  ...
  override implicit def makeAttrTestable[V](
      attr: HtmlAttr[V]
  ): TestableHtmlAttr[V] = {
    new TestableHtmlAttr[V](attr.name, attr.codec.encode, attr.codec.decode)
  }

  override implicit def makePropTestable[V, DomV](
  ...
  implicit def makeCompositePropTestable(
      prop: CompositeHtmlProp
  ): TestableProp[String, String] = {
    new TestableProp(prop.name, StringAsIsCodec.decode)
  }

  implicit def makeCompositeHtmlAttrTestable(
      attr: CompositeHtmlAttr
  ): TestableHtmlAttr[String] = {
    new TestableHtmlAttr(
      attr.name,
      StringAsIsCodec.encode,
      StringAsIsCodec.decode
    )
  }

  implicit def makeCompositeSvgAttrTestable(
      attr: CompositeSvgAttr
  ): TestableSvgAttr[String] = {
    new TestableSvgAttr(
      attr.name,
      StringAsIsCodec.encode,
      StringAsIsCodec.decode,
